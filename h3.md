...
Package-file-service: I'll start the service, and I have to watch the whole
directory now for changes. A rather old thread, but makes sense to me.

https://stackoverflow.com/questions/23716009/saltstack-in-a-watch-statement-how-do-i-specify-a-directory-where-all-files-sh

Adding the service part for the ufw/init.sls
```
firewall:
  - service.running:
    - name: ufw
    - watch:
      - file: /etc/ufw/*
```
Testing 2 things at once, but I'll remove the test file to see what happens:

```
min-5:
    Data failed to compile:
----------
    ID firewall in SLS ufw is not a dictionary
```

Didn't get it right. That service.running does not need a dash before it.
```
firewall:
  service.running:
    - name: ufw
    - watch:
      - file: /etc/ufw/*
```
Second test, removing the test file from minion first:
```
# salt '*' state.apply ufw >> h3.md
min-5:
----------
...
------------
Total states run:     3
Total run time: 258.117 ms
```
That old stackoverflow thread... I'll check the Internet:

https://groups.google.com/forum/#!topic/salt-users/kGQ5o4umNmc
https://docs.saltstack.com/en/latest/topics/best_practices.html

This is interesting, maybe the next one could work:
...
min-5:
    Data failed to compile:
----------
    Rendering SLS 'base:ufw' failed: could not find expected ':'
